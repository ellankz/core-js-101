...
class Builder {
  ...
  stringify() {
    const res = Builder.createSelector(this.selector);
    this.resetData();
    return res;
  }
  ...
  element(value) {
    if (
      this.selector.id !== ''
      || this.selector.class.length > 0
      || this.selector.attr.length > 0
      || this.selector.pseudoClass.length > 0
      || this.selector.pseudoElement !== ''
    ) {
      this.resetData();
...
const cssSelectorBuilder = {
  ...
  combine(selector1, combinator, selector2) {
    ...
    const res = [sel1, combinator, sel2].join(' ');
    return {
      stringify() {
        return res;
      },
